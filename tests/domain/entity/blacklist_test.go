...
func TestBlackList_NewBlackList(t *testing.T) {
	...
	newBlackList := fixtures.CreateBlacklist(10, &enventId, "reason", "document", entity.SPECIFIC, entity.TEMPORARY, &dateEvent)
...
func TestBlackList_NewBlackList_scope_invalid(t *testing.T) {
	enventId := uuid.NewV4().String()
	newBlackList := fixtures.CreateBlacklist(10, &enventId, "reason", "document", entity.GLOBAL, entity.PERMANENT, nil)
	err := newBlackList.IsValid()
	require.NotNil(t, err)
	require.Equal(t, err.Error(), "for this type of blacklist, its scope cannot be different from specific")

	newBlackList = fixtures.CreateBlacklist(10, nil, "reason", "document", entity.SPECIFIC, entity.PERMANENT, nil)
	err = newBlackList.IsValid()
	require.NotNil(t, err)
	require.Equal(t, err.Error(), "for this type of blacklist, its scope cannot be different from global")
